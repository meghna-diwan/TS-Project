...
# ARIMA with xreg using oil price, promotion and perishable
```{r}
...
```{r}
pred2 <- forecast(sales_am, h = 20, xreg=cbind(fourier(sales_train.ts, K=5, h=20), x_sales_val))
smape = function(y_pred, y_actual) {
  error = mean(abs(y_pred - y_actual)/(abs(y_actual) + abs(y_pred)))
  return(error)
}
...
# SMAPE Across the bottom level
```{r}
smape.values <- data.frame()

# Calculating smape for each column
for (i in 1:45){
  smape.values <- rbind(smape.values, smape(val_pred[, i] ,val_actual[, i]))
}

as.vector(unlist(smape.values))
```
...
# Cross-Validation Function using Arima & Xreg
```{r}
lr.CV = function(y, window, h) {
  ...
  for (i in seq(1, nrow(y) - (window + h) + 1)) {
    ...
    train_sales = y[train_start:train_end, ]
    test_sales = y[test_start:test_end, ]
    
    # Transform Train Sales for Fourier
    minday = as.Date(train_sales$date[1])
    sales_train.ts = ts(train_sales$sales, start = c(year(minday), as.numeric(format(minday, "%j"))), freq = 365)
    
    # Transform Test Sales for Fourier
    minday = as.Date(test_sales$date[1])
    sales_test.ts = ts(test_sales$sales, start = c(year(minday), as.numeric(format(minday, "%j"))), freq = 365)
    
    # Split Xreg Vars
    x_var_train = x_var[train_start:train_end, ]
    x_var_test = x_var[test_start:test_end, ]
    
    # Forecast
    sales_am <- auto.arima(train_sales$sales, xreg=cbind(fourier(sales_train.ts, K=5), x_var_train))
    fc = forecast(sales_am, h = h, xreg=cbind(fourier(sales_train.ts, K=5, h=h), x_var_test))
    print(paste("Done", as.character(i)))

    acc = smape(fc$mean, test_sales$sales)
...
x_var <- data.matrix(within(sales, rm("sales", "date")))
cv.acc = lr.CV(y = sales, window = 1650, h = 20)
```
